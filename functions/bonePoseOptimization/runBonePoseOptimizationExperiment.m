...
% Create a fresh folder on every invocation; v03 does not search for older runs.
experimentFolder = createExperimentFolder( ...
    experimentSpec.experiment.outputFolder, experimentSpec.experiment.name);
...
[configCopied, copyMessage] = copyfile( ...
    experimentSpec.source.configFilePath, configSnapshotPath);
if ~configCopied
...
environment.seedReproducibility = ...
    'best_effort_when_internal_parfor_is_enabled';
...
maximumEvaluations = experimentPlan.numberOfRuns * ...
    experimentSpec.optimizer.maxFunctionEvaluations;
fprintf('Created experiment: %s\n', experimentFolder);
fprintf('%d combinations x %d seeds = %d optimization runs.\n', ...
    experimentPlan.numberOfCombinations, experimentPlan.numberOfSeeds, ...
    experimentPlan.numberOfRuns);
...
for combinationIndex = 1:experimentPlan.numberOfCombinations
    ...
    for runIndex = combinationRunIndexes(:).'
        ...
        [runResult, summaryRecord] = runOneSeed( ...
            runRow, seedConfig, seedData, validationReference, ...
            initialPoseVector, initialCost);
...
seedFolder = fullfile(experimentFolder, 'runs', ...
    char(runRow.combinationId), sprintf('seed_%d', runRow.seed));
...
function [runResult, summaryRecord] = runOneSeed( ...
        runRow, runConfig, data, validationReference, initialPoseVector, initialCost)
...
try
    ...
    optimizationResult = runBonePoseOptimization( ...
        initialPoseVector, data, runConfig, initialCost);
    % Reevaluate the best pose once because the optimizer normally keeps only scalar costs.
    [finalCost, finalCostDetails] = bonePoseCostFunction( ...
        optimizationResult.result.bestPoseVector, data, runConfig);
    runStatus = "completed";
...
runResult = createRunResultBase( ...
    runRow, runConfig, runStatus, startTime, endTime, runtimeSeconds);
...
function [runResult, summaryRecord] = createFailedRunResult( ...
        runRow, runConfig, caughtError, failureStage, initialCost, validationReference)
...
runResult = createRunResultBase( ...
    runRow, runConfig, "failed", eventTime, eventTime, 0);
...
function runResult = createRunResultBase( ...
        runRow, runConfig, status, startTime, endTime, runtimeSeconds)
...
validationReference.T_CT_ref_groundTruth = ...
    validationData.groundTruthBonePose.T_CT_ref;
validationReference.T_bone_ref_groundTruth = ...
    validationData.groundTruthBonePose.T_bone_ref;
end
...
if strcmp(runResult.run.status, 'completed')
    ...
    summaryRecord.functionEvaluations = ...
        runResult.optimizationResult.cmaes.counteval;
    summaryRecord.stopFlag = joinStopFlags( ...
        runResult.optimizationResult.cmaes.stopflag);
else
...
while isfolder(experimentFolder)
    ...
    experimentFolder = fullfile(outputFolder, ...
        sprintf('%s_%02d', baseName, collisionIndex));
end
