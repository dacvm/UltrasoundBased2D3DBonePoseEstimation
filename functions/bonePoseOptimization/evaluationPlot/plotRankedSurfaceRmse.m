...
figureHandle = figure('Name', 'Ranked Surface RMSE');
hold on;
for combinationIndex = 1:height(plotTable)
    plot([plotTable.q25SurfaceRmseMm(combinationIndex), ...
         ...
         [yPosition(combinationIndex), ...
         yPosition(combinationIndex)], ...
         '-', ...
         'Color', [0.35 0.35 0.35], ...
         'LineWidth', 1.5);
end
scatter(plotTable.medianSurfaceRmseMm, yPosition, 42, [0.12 0.47 0.71], 'filled');
hold off;

grid on;
set(gca, 'YDir', 'reverse', ...
    'YTick', yPosition, ...
    'YTickLabel', cellstr(erase(plotTable.combinationId, "combination_")));
