function [CA,sectionCenters,CoM,V_csL,CAhulltot,secI] = computeCrossSectionAreaAlongAxis_fem(V,R_axes,stepSize_perc)
...
i = max(secI):-1:1;
CAhulltot = zeros(50,length(i));

for i = max(secI):-1:1
    theseV = V23(secI==i,:) ;
    if size(theseV,1)>2
        [CAhull,CA(i)] = convhull(theseV);
    else
        CA(i) = 0 ;
        CAhull = 0 ;
    end
    CAhulltot(1:length(CAhull),i) = CAhull;
end
