...
% Move the femoral landmark-ray origin distally from the femoral ACS origin.
% The default keeps the rays at the original epicondylar-axis location.
femurEpicondyle_distalOffset_mm = 0;
...
% The femoral positive y-axis is proximal, so only nonnegative offsets have
% the requested meaning of moving the ray origin distally along negative y.
if ~isnumeric(femurEpicondyle_distalOffset_mm) || ...
        ~isscalar(femurEpicondyle_distalOffset_mm) || ...
        ~isreal(femurEpicondyle_distalOffset_mm) || ...
        ~isfinite(femurEpicondyle_distalOffset_mm) || ...
        femurEpicondyle_distalOffset_mm < 0
    error('bonePreRegistration:InvalidFemurDistalOffset', ...
        'femurEpicondyle_distalOffset_mm must be one finite, nonnegative number.');
end
...
% The ERC positive y-axis is proximal for the femur as well as the tibia, so
% subtracting the requested distance moves the ray origin distally.
...
point_femurLineOrigin_CT = point_femurAcsOrigin_CT - femurEpicondyle_distalOffset_mm * vector_femurY_CT;
...
landmarks(1).distalOffset_mm = femurEpicondyle_distalOffset_mm;
...
intersectionDiagnostics(1).distalOffset_mm    = femurEpicondyle_distalOffset_mm;
...
fprintf('Femoral landmark width at %.2f mm distal and %.2f deg anterior: %.2f mm\n', ...
    femurEpicondyle_distalOffset_mm, femurEpicondyle_anteriorAngleOffset_deg, ...
...
% Show each distal shift as a dashed segment from its true ACS origin to its
% paired-ray origin. A zero offset naturally produces an invisible segment.
plot3(axes_landmarks, ...
...
title(axes_landmarks, sprintf( ...
    ['%s knee landmarks | femur %.1f mm, %.1f deg | ', ...
     'tibia %.1f mm, %.1f deg'], ...
    upper(kneeSide), femurEpicondyle_distalOffset_mm, ...
    ...
    if isempty(forwardDistances)
        ...
        error('bonePreRegistration:NoRayMeshIntersection', ...
            ['The %s %s landmark ray does not intersect the mesh in its ', ...
             ...
             'distal or anterior angle offset.'], ...
...
if isempty(negativeIndices) || isempty(positiveIndices)
    error('bonePreRegistration:MissingOppositeSideIntersections', ...
        ['The %s landmark line must cross the mesh on both sides of its ', ...
         'origin. Check the ACS and reduce that bone''s distal or anterior ', ...
         'angle offset.'], ...
        meshLabel);
end
