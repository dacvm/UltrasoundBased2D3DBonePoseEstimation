...
verifyFalse(testCase, ...
    disabledMetadata.resolvedConfiguration.regularization.enabled);
end
...
verifyLessThanOrEqual(testCase, abs(displacementMm(isfinite(surfaceRows))), ...
    ...
    regularization.maximumDisplacementMm + 1e-8);
...
configurationGroups = {'imageEvidence', 'surfaceTracing', ...
    'gapInterpolation', 'regularization'};
verifyTrue(testCase, all(isfield( ...
    extractionMetadata.resolvedConfiguration, configurationGroups)));
regularizationFields = {'enabled', 'halfResponseWavelengthMm', ...
    'huberDeltaMm', 'maximumDisplacementMm', 'minimumDataWeight', ...
    'maximumIterations', 'convergenceMm'};
verifyTrue(testCase, all(isfield( ...
    extractionMetadata.resolvedConfiguration.regularization, ...
    regularizationFields)));
verifyTrue(testCase, ...
    extractionMetadata.resolvedConfiguration.regularization.enabled);
verifyEqual(testCase, extractionMetadata.resolvedConfiguration. ...
    regularization.halfResponseWavelengthMm, 2.5);
verifyEqual(testCase, extractionMetadata.resolvedConfiguration. ...
    regularization.huberDeltaMm, 0.15);
verifyEqual(testCase, extractionMetadata.resolvedConfiguration. ...
    regularization.maximumDisplacementMm, 0.75);
verifyEqual(testCase, extractionMetadata.resolvedConfiguration. ...
    regularization.minimumDataWeight, 0.10);
verifyEqual(testCase, extractionMetadata.resolvedConfiguration. ...
    regularization.maximumIterations, 10);
verifyEqual(testCase, extractionMetadata.resolvedConfiguration. ...
    regularization.convergenceMm, 0.001);
