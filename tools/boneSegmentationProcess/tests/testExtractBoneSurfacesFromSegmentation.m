...
verifyEqual(testCase, metadata.algorithmVersion, '1.3.0');
end
...
verifySize(testCase, surfaceResults(1).surfaceNormalXY, [0, 2]);
verifySize(testCase, surfaceResults(1).surfaceNormalMask, [0, 1]);
...
verifySize(testCase, surfaceResults(2).surfaceNormalXY, [0, 2]);
verifySize(testCase, surfaceResults(2).surfaceNormalMask, [0, 1]);
end
...
numberOfCoordinateRows = size(surfaceResults.surfaceCoordinatesXY, 1);
verifySize(testCase, surfaceResults.surfaceNormalXY, ...
    [numberOfCoordinateRows, 2]);
verifySize(testCase, surfaceResults.surfaceNormalMask, ...
    [numberOfCoordinateRows, 1]);
verifyClass(testCase, surfaceResults.surfaceNormalMask, 'logical');
...
verifyEqual(testCase, ...
    resultFieldNames{surfaceCoordinateFieldIndex + 2}, ...
    'surfaceNormalXY');
verifyEqual(testCase, ...
    resultFieldNames{surfaceCoordinateFieldIndex + 3}, ...
    'surfaceNormalMask');
...
% Normals share the compact coordinate row order. Every valid vector is
% physical-unit length and probe-facing; any invalid row uses the NaN sentinel.
normalValues = surfaceResults.surfaceNormalXY;
normalMask = surfaceResults.surfaceNormalMask;
verifySize(testCase, normalValues, size(coordinates));
verifySize(testCase, normalMask, [size(coordinates, 1), 1]);
verifyClass(testCase, normalMask, 'logical');
verifyTrue(testCase, all(isfinite(normalValues(normalMask, :)), 'all'));
verifyEqual(testCase, vecnorm(normalValues(normalMask, :), 2, 2), ...
    ones(nnz(normalMask), 1), 'AbsTol', 1e-10);
verifyLessThanOrEqual(testCase, normalValues(normalMask, 2), 0);
verifyTrue(testCase, all(isnan(normalValues(~normalMask, :)), 'all'));
...
verifyEqual(testCase, string(extractionMetadata.algorithmVersion), "1.3.0");
verifyEqual(testCase, extractionMetadata.numberOfFrames, 1);
verifyEqual(testCase, ...
    string(extractionMetadata.normalConvention.coordinateFrame), ...
    "imagePhysicalXY");
verifyEqual(testCase, ...
    string(extractionMetadata.normalConvention.coordinateOrder), ["x", "y"]);
verifyTrue(testCase, extractionMetadata.normalConvention.unitLength);
verifyEqual(testCase, ...
    string(extractionMetadata.normalConvention.polarity), "towardProbe");
verifyEqual(testCase, ...
    string(extractionMetadata.normalConvention.probeDirection), ...
    "decreasingImageY");
verifyEqual(testCase, ...
    string(extractionMetadata.normalConvention.estimator), ...
    "segmentWiseCentralDifference");
