function pdTree = buildPIMLOPPDTree_batchedProcess(boneMeshCT, validFaceMask, options)
%BUILDPIMLOPPDTREE_BATCHEDPROCESS Build a PD-tree with packed node geometry.
...
%       batchGeometry    - packed rotations, origins, and bounds for query batches;
...
%       pdTree = buildPIMLOPPDTree_batchedProcess( ...
...
% Pack node geometry once so a batch search can access one node without
% repeatedly unpacking its transform. The tree construction above remains
% the paper's standard PCA split; batching changes how queries use the tree.
nodeTransformsCT = cat(3, nodes.T_node_CT);
pdTree.batchGeometry.rotationsNodeToCT = nodeTransformsCT(1:3, 1:3, :);
pdTree.batchGeometry.originsCT = reshape(nodeTransformsCT(1:3, 4, :), 3, []).';
pdTree.batchGeometry.boundsMinNode = vertcat(nodes.boundsMinNode);
pdTree.batchGeometry.boundsMaxNode = vertcat(nodes.boundsMaxNode);
