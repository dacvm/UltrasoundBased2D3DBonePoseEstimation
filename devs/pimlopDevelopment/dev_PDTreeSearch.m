...
%% PREPARE THE WHOLE BONE MODEL IN THE REFERENCE FRAME
...
bonePointsRef    = applyRigidTransform(boneMeshCT.Points, T_CT_ref_initial);
boneMeshRef      = triangulation(boneMeshCT.ConnectivityList, bonePointsRef);

% Psi is the complete model search domain from the P-IMLOP notation. An
% individual candidate y may lie anywhere on one of these triangles; it is
% not restricted to the stored mesh vertices. Thus, this setup represents
% one measured oriented point X against many possible model points y in Psi.
Psi = boneMeshRef;
