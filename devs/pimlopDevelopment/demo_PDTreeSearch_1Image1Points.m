...
% matchDetails contains the normalized projection of the selected model
% normal onto the local ultrasound image X-Y plane. This is the exact green
% direction used in the orientation part of E_match. Append a zero image-Z
% component, then rotate the direction into ref so it can be drawn on the
% tracked ultrasound plane. Its origin will be X, where this direction is
% compared with the measured red normal.
projectedYNormal3DRef = R_image_ref * [ ...
    matchDetails.projectedYNormal2DImage; 0];
...
% Draw the projected model normal in green from X. Its image-Z component was
% removed above, so this arrow lies exactly in the ultrasound image plane.
% Comparing the green arrow with the red measured normal shows the angular
% disagreement that contributes to the P-IMLOP orientation error.
projectedYNormalHandle = quiver3(demoAxes, ...
    X.position3DRef(1), X.position3DRef(2), X.position3DRef(3), ...
    projectedYNormal3DRef(1) * normalDisplayScale, ...
    projectedYNormal3DRef(2) * normalDisplayScale, ...
    projectedYNormal3DRef(3) * normalDisplayScale, ...
    0, 'Color', [0.05, 0.70, 0.20], 'LineWidth', 2.0, ...
    'MaxHeadSize', 0.8, 'DisplayName', 'Projected model normal');
