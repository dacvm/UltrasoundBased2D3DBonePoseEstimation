function PsiCT = preparePIMLOPModel_batchedProcess(boneMeshCT)
%PREPAREPIMLOPMODEL_BATCHEDPROCESS Prepare geometry and cached triangle arrays.
...
%       triangleVerticesCT - 3-by-3-by-M cached complete triangles in CT;
...
if ~isa(boneMeshCT, 'triangulation')
    error('preparePIMLOPModel_batchedProcess:InvalidMesh', ...
...
if any(~isfinite(bonePointsCT), 'all')
    error('preparePIMLOPModel_batchedProcess:NonfiniteVertex', ...
...
if ~any(validFaceMask)
    error('preparePIMLOPModel_batchedProcess:NoValidFaces', ...
...
% Cache all complete triangles in one numeric array for leaf batches.
% Page j is a 3-by-3 triangle (rows are vertices) with original face index j.
% This geometry is fixed in CT and survives every optimizer candidate.
PsiCT.triangleVerticesCT = permute(cat(3, faceVertex1CT, faceVertex2CT, faceVertex3CT), [3, 2, 1]);
PsiCT.pdTree = [];
end
